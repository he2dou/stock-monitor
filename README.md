...
内置 Web 监控后台，基于 FastAPI + Jinja2 服务端渲染，无需额外前端构建。

### 启动方式

**嵌入式（推荐）**：监控调度器与 Web 服务在同一进程运行，共享 SQLite 存储，可在页面上触发“立即运行”。

```bash
python -m src.main
```

启动后同时运行定时行情抓取（按 `monitor.interval_minutes`）和 Web 监控后台。访问 `http://127.0.0.1:8000`。

**独立模式**：仅启动 Web 界面，不运行监控调度。适合已有独立监控进程的场景。
...
在 `config/config.yaml` 的 `web` 段配置：

```yaml
web:
  host: "127.0.0.1"     # 监听地址
  port: 8000             # 监听端口
  secret_key: ""         # Session 密钥，留空则每次启动随机生成
  admin_password: ""     # 管理密码，留空则无需登录（本地开发模式）
```

设置 `admin_password` 后所有页面需登录访问；留空则开放访问。

### 功能页面

| 路径 | 说明 |
|------|------|
| `/` | 概览：账户余额、持仓、最新行情、大盘指数 |
| `/watchlist` | 股票池：增删启停监控标的 |
| `/alerts` | 预警：增删启停预警规则 |
| `/markets` | 行情：查看快照报价；`/markets/history/{symbol}` 查看历史走势 |
| `/portfolio` | 持仓：账户余额、持仓、挂单、成交记录、交易信号 |
